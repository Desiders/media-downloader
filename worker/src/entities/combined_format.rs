use super::format::{Audio, Kind, Video};
...
#[derive(Clone, Debug)]
pub struct Format<'a> {
    pub video_format: Video<'a>,
    pub audio_format: Audio<'a>,
}

impl<'a> Format<'a> {
    #[must_use]
    pub fn new(video_format: Video<'a>, audio_format: Audio<'a>) -> Self {
        Self {
            video_format,
            audio_format,
        }
...
impl<'a> From<Vec<Kind<'a>>> for Formats<'a> {
    fn from(formats: Vec<Kind<'a>>) -> Self {
        ...
        for format in formats {
            match format {
                Kind::Video(video_format) => {
                    video_formats.push(video_format);
                }
                Kind::Audio(audio_format) => {
                    audio_formats.push(audio_format);
                }
                Kind::Combined(audio_format, video_format) => {
...
impl<'a> From<Option<Vec<Kind<'a>>>> for Formats<'a> {
    fn from(formats: Option<Vec<Kind<'a>>>) -> Self {
        match formats {
